...
export function ExternalStatsCards() {
  ...
  useEffect(() => {
    async function cargarSolicitudes() {
      try {
        ...
        console.error(
          "Error al cargar las solicitudes para las estadísticas:",
          error
        )
  ...
  const animalesExternos = useMemo(() => {
  ...
  solicitudes
    ...
    return animales.filter((animal) =>
      idsExternos.has(Number(animal.id))
    )
  }, [animales, solicitudes])

  const totalAnimales = animalesExternos.length

  const enAdopcion = animalesExternos.filter(
    (animal) => animal.estado === "disponible"
  ).length

  const adoptados = animalesExternos.filter(
    (animal) => animal.estado === "adoptado"
  ).length

  const stats = [
    {
      title: "Total de Animales Externos",
      value: totalAnimales,
      icon: PawPrintIcon,
      color: "bg-[#2563EB]",
